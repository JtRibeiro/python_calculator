...
class Calculator3:
  ...
  def calculate(self, request: FlaskRequest) -> Dict:  # type: ignore
    ...
    multiplication = self.__calculate_multipliaction(input_data)
  ...
  def __calculate_multipliaction(self, numbers: List[float]) -> float:
    multiplication = 1
  ...
  def __verify_results(self, variance:float, multiplication: float) -> None:
    if variance < multiplication: 
      raise HttpBadRequestError("Falha no processo: Variância menor que o multiplicação")
